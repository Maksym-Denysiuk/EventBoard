...
- This applies regardless of whether the run finds any new events — even a run that discovers nothing must still drop expired entries from `Storage/events.json`.
- Default behavior: keep the file limited to upcoming events only (forward-looking timeline use case). Only retain past events if explicitly configured elsewhere to keep a historical log.
- Note in the run's working notes how many events were pruned and their names/dates, so it's traceable in the completion summary.

### 6. Deduplicate (before writing)
- Merge: existing events (already pruned of past-dated entries in Step 5) + newly formatted events.
...
5a. **Prune past-dated events every run.** Step 5 hard-deletes any stored event whose date is already behind the current run date, before dedup/write — this happens even on a run that finds zero new events.
